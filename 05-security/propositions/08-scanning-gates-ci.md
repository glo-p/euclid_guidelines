...
Def. V.10, Def. I.20, Post. I.6, Post. V.4, Post. I.1, CN 7.

## Demonstration

The attack surface (Def. V.10) includes the dependencies a service executes and the image and infrastructure it runs on, none of which the owning team wrote. Those parts change beneath the team without a commit, so only a machine check at each build sees the current state (Post. I.6). Post. V.4 is itself unverifiable without IaC scanning, since an over-broad IAM policy is invisible in a code review. A suppression that is not recorded is, by Def. I.20, a defect rather than an exception, and by CN 7 an unobserved vulnerability is one the company does not know it has. ∎ Q.E.D.
