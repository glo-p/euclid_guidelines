...
| `POST` | create item in collection, or perform an action | no | no (unless Idempotency-Key, [Prop. II.8](08-idempotency.md)) | `201` + `Location` on create; `200` for an action with a result; `202` for long-running ([Prop. II.15](15-long-running-operations.md)) |
...
[Post. II.1](../postulates.md#Post.%20II.1%20-%20HTTP%20Is%20Authoritative), [Post. I.4](../../01-foundations/postulates.md#Post.%20I.4%20-%20Unreliable%20Network), [Def. I.16](../../01-foundations/definitions.md#Def.%20I.16%20-%20Idempotent%20Operation), [Def. II.8](../definitions.md#Def.%20II.8%20-%20Problem), [CN 1](../../01-foundations/common-notions.md#CN%201%20-%20Substitutability), [CN 3](../../01-foundations/common-notions.md#CN%203%20-%20Explicit%20Is%20Greater%20Than%20Implicit).

## Demonstration

[Post. II.1](../postulates.md#Post.%20II.1%20-%20HTTP%20Is%20Authoritative) fixes the meaning of methods and codes; a producer that uses them otherwise is not speaking HTTP, and generic clients, caches, the edge and observability tooling (which all assume HTTP) will misbehave. Under [Post. I.4](../../01-foundations/postulates.md#Post.%20I.4%20-%20Unreliable%20Network) a client must decide whether to retry; the safe/idempotent columns are exactly the information it needs, so the method must be honest about them ([Def. I.16](../../01-foundations/definitions.md#Def.%20I.16%20-%20Idempotent%20Operation)). Returning `200` for a failure hides the failure from every layer that reads status codes, contradicting [CN 3](../../01-foundations/common-notions.md#CN%203%20-%20Explicit%20Is%20Greater%20Than%20Implicit). Two partial-update grammars would violate [CN 5](../../01-foundations/common-notions.md#CN%205%20-%20One%20Concept%2C%20One%20Shape), so one is chosen; Merge Patch is chosen because it is the representation itself with fields omitted, which any consumer can produce without a library. ∎ Q.E.D.

## Corollaries

* **Cor. II.3.1** - `404` is returned for a resource that exists but belongs to another tenant, never `403`, so that existence is not disclosed across tenants ([Book V](../../05-security/README.md)).
* **Cor. II.3.2** - `422` is for requests that pass schema validation but fail a business rule; `400` is for requests that fail schema validation. The Problem `type` ([Prop. II.4](04-problem-details.md)) distinguishes the rule.
* **Cor. II.3.3** - `GET` and `DELETE` never have request bodies.

## Construction

* ASP.NET Core: enable `ProblemDetails` services ([Prop. II.4](04-problem-details.md)) so every listed condition produces the right code with a Problem body.
...
The one genuinely debatable row is `POST` for actions. It is the least worst option: `PUT` is idempotent, which an action often is not; inventing a method is not HTTP. See [Prop. II.2](02-resource-naming.md) for the sub-resource form that keeps it honest.
