...
A service calling another service's API MUST authenticate as its own principal (Def. V.1) using an OAuth2 client-credentials access token issued by the identity provider (Post. V.1). A service MAY instead use AWS IAM SigV4 request signing where both parties are in AWS and the target is a private integration; this alternative is an open question and MUST NOT be adopted without an ADR. A service MUST NOT forward a user's access token to another service as its own credential.

## Given

Def. V.1, Def. V.3, Post. V.1, Post. V.2, Post. V.4, Prop. V.1, Prop. V.9, CN 3.

## Demonstration

Every request is performed by exactly one principal (Def. V.1), and a service is a principal. By Post. V.1 the identity provider issues every token, so a service obtains its own token from it, and client credentials is the OIDC grant for a non-human principal. Forwarding the user's token would attribute the downstream action to the user rather than the calling service, making the audit event (Prop. V.9) untrue. SigV4 binds identity to IAM (Post. V.4) and fits private integrations, but it bypasses the identity provider, so it is admitted only by recorded decision. ∎ Q.E.D.
...
* Client registration per service in the identity provider; client secret stored in AWS Secrets Manager (Prop. V.4) and rotated by Secrets Manager rotation.
...
* API Gateway JWT authorizer accepting service audiences; scopes per Prop. V.2.
